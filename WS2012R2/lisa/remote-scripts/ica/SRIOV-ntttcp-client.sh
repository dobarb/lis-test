...
redhat_5|redhat_6|centos_6)
    if [ "$DISTRO" == "redhat_5" ] || ["$DISTRO" == "centos_6" ]; then
        upgrade_gcc
        if [ $? -ne 0 ]; then
...
LogMsg "Copy files to server: ${BOND_IP2}"
scp -i $HOME/.ssh/$sshKey -v -o StrictHostKeyChecking=no ~/SRIOV-ntttcp-server.sh ${REMOTE_USER}@[${BOND_IP2}]:
if [ $? -ne 0 ]; then
    msg="ERROR: Unable to copy test scripts to target server machine: ${BOND_IP2}. scp command failed."
    LogMsg "${msg}"
    echo "${msg}" >> ~/summary.log
    UpdateTestState $ICA_TESTFAILED
    exit 130
fi
scp -i $HOME/.ssh/$sshKey -v -o StrictHostKeyChecking=no ~/constants.sh ${REMOTE_USER}@[${BOND_IP2}]:
scp -i $HOME/.ssh/$sshKey -v -o StrictHostKeyChecking=no ~/utils.sh ${REMOTE_USER}@[${BOND_IP2}]:
scp -i $HOME/.ssh/$sshKey -v -o StrictHostKeyChecking=no ~/perf_utils.sh ${REMOTE_USER}@[${BOND_IP2}]:
...
ssh -i $HOME/.ssh/$sshKey -v -o StrictHostKeyChecking=no ${REMOTE_USER}@${BOND_IP2} "echo '~/SRIOV-ntttcp-server.sh > ntttcp_ServerSideScript.log' | at now"
if [ $? -ne 0 ]; then
...
while [ $wait_for_server -gt 0 ]; do
    ...
    scp -i $HOME/.ssh/$sshKey -v -o StrictHostKeyChecking=no ${REMOTE_USER}@[${BOND_IP2}]:~/state.txt ~/${server_state_file}
...
ssh -i $HOME/.ssh/${sshKey} -v -o StrictHostKeyChecking=no ${REMOTE_USER}@${BOND_IP2} "mkdir /root/$log_folder"
...
do
    ...
    ssh -i $HOME/.ssh/${sshKey} -v -o StrictHostKeyChecking=no ${REMOTE_USER}@${BOND_IP2} "pkill -f ntttcp"
    ssh -i $HOME/.ssh/${sshKey} -v -o StrictHostKeyChecking=no ${REMOTE_USER}@${BOND_IP2} "ntttcp -r${BOND_IP2} -P $num_threads_P ${ipVersion} -e > $HOME/$log_folder/ntttcp-receiver-p${num_threads_P}X${num_threads_n}.log" &

    ssh -i $HOME/.ssh/${sshKey} -v -o StrictHostKeyChecking=no ${REMOTE_USER}@${BOND_IP2} "pkill -f lagscope"
    ssh -i $HOME/.ssh/${sshKey} -v -o StrictHostKeyChecking=no ${REMOTE_USER}@${BOND_IP2} "lagscope -r${BOND_IP2} ${ipVersion}" &
...
if [ $sts -eq 0 ]; then 
    ...
    cd $HOME
    scp -i $HOME/.ssh/${sshKey} -v -o StrictHostKeyChecking=no ${REMOTE_USER}@${BOND_IP2}:/root/$log_folder/* /root/$log_folder
    if [ $? -ne 0 ]; then
